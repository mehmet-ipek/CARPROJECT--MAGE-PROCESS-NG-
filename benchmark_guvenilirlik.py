...
LOG_DIR = "benchmark_guvenilirlik_logs"
os.makedirs(LOG_DIR, exist_ok=True)

# Hedef sınıf eşleştirmesi (modelin kendi .names'indeki isim -> Türkçe etiket)
TARGET_CLASSES = {
    "car": "Araba",
    "cars": "Araba",
    "motorcycle": "Motor",
...
def get_class_name_from_model(model, cls_id):
    raw_name = model.names.get(int(cls_id), None)
    if raw_name is None:
        yedek = {0: "Araba", 1: "Motor", 2: "Otobus", 3: "Kamyonet"}
        return yedek.get(int(cls_id), f"ID:{cls_id}")
...
class LKWrapper:
    ...
    def update(self, frame):
        # 1. Modelin ham tespitlerini al (maske dedektör içinde uygulanıyor)
        ...
        # 3. Yeni tespitleri mevcut kutularla eşleştir
...
def benchmark_model(detector, model_name, video_path, max_frames=300):
    ...
    with open(log_path, 'w', encoding='utf-8') as log_file:
        ...
        total_boxes = 0
        unique_ids = set()
        track_class_history = {}   # track_id -> Counter({TürkçeEtiket: count})
        track_conf_history = {}    # track_id -> list of confidences
        fps_values = []
        ...
        while frame_count < max_frames:
            ...
            for track_id, box_data in tracked.items():
                ...
                cls_name = get_class_name_from_model(detector.model, cls_id)

                if track_id not in track_class_history:
                    track_class_history[track_id] = Counter()
                    track_conf_history[track_id] = []
                track_class_history[track_id][cls_name] += 1
                track_conf_history[track_id].append(conf)

                log_file.write(f"  ID:{track_id} | cls:{cls_id} ({cls_name}) | conf:{conf:.3f} | "
                               f"box=({x1},{y1},{bw},{bh})\n")
        ...
        # Sınıf tutarlılığı ve ortalama güven
        consistencies = []
        track_avg_confs = []
        for tid in track_class_history:
            counter = track_class_history[tid]
            total = sum(counter.values())
            if total > 0:
                dominant_count = counter.most_common(1)[0][1]
                consistency = dominant_count / total
                consistencies.append(consistency)
            if tid in track_conf_history and track_conf_history[tid]:
                track_avg_confs.append(np.mean(track_conf_history[tid]))

        avg_consistency = np.mean(consistencies) if consistencies else 0.0
        avg_confidence = np.mean(track_avg_confs) if track_avg_confs else 0.0

        # Baskın sınıf dağılımı
        final_class_counts = Counter()
        for tid, counter in track_class_history.items():
            dominant_name = counter.most_common(1)[0][0]
            final_class_counts[dominant_name] += 1

        log_file.write("\n" + "="*80 + "\n")
        log_file.write("GÜVENİLİRLİK RAPORU\n")
        log_file.write(f"Frame: {frame_count}\n")
        log_file.write(f"Benzersiz araç: {len(unique_ids)}\n")
        log_file.write(f"Toplam kutu: {total_boxes}\n")
        log_file.write(f"Ortalama FPS: {np.mean(fps_values):.2f}\n" if fps_values else "FPS: N/A\n")
        log_file.write(f"Ortalama Güven Skoru (araç başına): {avg_confidence:.3f}\n")
        log_file.write(f"Ortalama Sınıf Tutarlılığı: %{avg_consistency*100:.1f}\n")
        log_file.write("Sınıf dağılımı (benzersiz araçlar):\n")
        for cls_name, count in sorted(final_class_counts.items()):
            log_file.write(f"  {cls_name}: {count}\n")
...
def plot_results(results):
    ...
    conf_vals = [res['avg_confidence'] for res in results.values()]
    consist_vals = [res['avg_consistency'] for res in results.values()]

    plt.figure(figsize=(16, 10))

    plt.subplot(2, 3, 1)
    plt.bar(models, unique_vals, color='steelblue')
    ...
    plt.subplot(2, 3, 4)
    plt.bar(models, consist_vals, color='crimson')
    plt.title('Sınıf Tutarlılığı (0-1)')
    plt.xticks(rotation=15)
    ...
    for i, cls in enumerate(all_classes):
        ...
        plt.bar(models, counts, bottom=bottom, label=cls, color=colors[i % len(colors)])
    ...
    plt.tight_layout()
    plt.savefig('benchmark_guvenilirlik.png', dpi=150)
    plt.close()
    print("\n✅ Grafik 'benchmark_guvenilirlik.png' olarak kaydedildi.")
...
def main():
    ...
    for name, det in models.items():
        ...
        print(f"  Benzersiz: {res['unique_vehicles']}, Kutu: {res['total_boxes']}, "
              f"Güven: {res['avg_confidence']:.3f}, Tutarlılık: %{res['avg_consistency']*100:.1f}")

    print("\n" + "="*100)
    print(f"{'Model':<25} {'Benzersiz':<10} {'Kutu':<10} {'Ort.Güven':<10} {'Tutarlılık':<10} {'FPS':<8}")
    print("-"*100)
    for name, res in results.items():
        print(f"{name:<25} {res['unique_vehicles']:<10} {res['total_boxes']:<10} "
              f"{res['avg_confidence']:<10.3f} %{res['avg_consistency']*100:<10.1f} {res['avg_fps']:<8.1f}")
    ...
    for name, res in results.items():
        ...
        if not res['class_counts']:
            ...
            continue
        for cls_name, count in sorted(res['class_counts'].items()):
