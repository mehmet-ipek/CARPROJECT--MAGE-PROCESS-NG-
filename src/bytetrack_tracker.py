import cv2
import numpy as np

class ByteTrackWrapper:

    def __init__(self, yolo_detector):
        self.model = yolo_detector.model
        self.allowed_classes = yolo_detector.allowed_classes
        self.min_width = 2
        self.min_height = 2

        # Maske yükle (siyah beyaz trafikvideo.jpg)
        mask_raw = cv2.imread("trafikvideo.jpg", cv2.IMREAD_GRAYSCALE)
        if mask_raw is not None:
            _, self.mask_bin = cv2.threshold(mask_raw, 100, 255, cv2.THRESH_BINARY)
            kernel = np.ones((5, 5), np.uint8)
            self.mask_bin = cv2.dilate(self.mask_bin, kernel, iterations=2)
            print("[BYTETRACK] Maske yuklendi ve genisletildi: trafikvideo.jpg")
        else:
            self.mask_bin = None
            print("[BYTETRACK] UYARI: trafikvideo.jpg bulunamadi. Maske devre disi.")

        print("\n======================================")
        print("[BYTETRACK] Model basariyla baglandi.")
        print("[BYTETRACK] Model siniflari:")
        print(self.model.names)
        print("[BYTETRACK] Izin verilen class ID'leri:")
        print(self.allowed_classes)
        print(f"[BYTETRACK] Min boyut: {self.min_width}x{self.min_height}")
        print("======================================\n")

    def update(self, frame):
        # Güveni artırılmış model için eşik 0.30
        conf_thresh = 0.10

        results = self.model.track(
            source=frame,
            conf=conf_thresh,
            classes=self.allowed_classes,
            imgsz=640,
            max_det=500,
            agnostic_nms=False,
            tracker="bytetrack.yaml",
            persist=True,
            verbose=False
        )

        tracked_objects = {}
        frame_h, frame_w = frame.shape[:2]

        # Maskeyi frame boyutuna getir
        if self.mask_bin is not None:
            current_mask = cv2.resize(self.mask_bin, (frame_w, frame_h))
        else:
            current_mask = np.full((frame_h, frame_w), 255, dtype=np.uint8)

        for result in results:
            if result.boxes is None:
                continue

            for box in result.boxes:
                if box.id is None:
                    continue

                track_id = int(box.id[0].cpu().numpy())
                cls_id = int(box.cls[0].cpu().numpy())
                conf = float(box.conf[0].cpu().numpy())

                class_name = self.model.names.get(cls_id, f"Bilinmeyen({cls_id})")

                x1, y1, x2, y2 = box.xyxy[0].cpu().numpy()
                x1 = int(max(0, x1))
                y1 = int(max(0, y1))
                x2 = int(min(frame_w - 1, x2))
                y2 = int(min(frame_h - 1, y2))

                bw = x2 - x1
                bh = y2 - y1

                if bw < self.min_width or bh < self.min_height:
                    continue
                if bw > frame_w * 0.95 or bh > frame_h * 0.95:
                    continue

                cx = int(x1 + (bw / 2))
                cy = int(y1 + (bh / 2))
                if not (0 <= cx < frame_w and 0 <= cy < frame_h):
                    continue

                # Maske kontrolü
                if current_mask[cy, cx] == 0:
                    continue

                print(f"[DETECT] ID={track_id} | CLASS_ID={cls_id} | CLASS={class_name} | CONF={conf:.3f} | BOX=({x1},{y1},{bw},{bh})")

                tracked_objects[track_id] = (int(x1), int(y1), int(bw), int(bh), cls_id, conf)

        return tracked_objects