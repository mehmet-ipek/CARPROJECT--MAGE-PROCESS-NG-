...
LOG_DIR = "benchmark_logs_lk_guven"
os.makedirs(LOG_DIR, exist_ok=True)

# Hedef sınıf eşleştirmesi
TARGET_CLASSES = {
    "car": "Araba",
    "cars": "Araba",
    "motorcycle": "Motor",
...
def benchmark_model(detector, model_name, video_path, max_frames=300):
    ...
    with open(log_path, 'w', encoding='utf-8') as log_file:
        ...
        for tid in track_class_history:
            total = sum(track_class_history[tid].values())
            if total > 0:
                dominant_count = track_class_history[tid].most_common(1)[0][1]
                consistencies.append(dominant_count / total)
        ...
        log_file.write(f"Ortalama Güven Skoru: {avg_confidence:.3f}\n")
...
def plot_results(results):
    ...
    plt.tight_layout()
    plt.savefig('benchmark_lk_guvenilirlik.png', dpi=150)
    plt.close()
    print("\n✅ Grafik 'benchmark_lk_guvenilirlik.png' olarak kaydedildi.")
...
def main():
    ...
    for name, det in models.items():
        print(f"\n--- {name} test ediliyor (LK Optik Akış, conf=0.30) ---")
