...
LOG_DIR = "benchmark_logs_lk_dogru"
os.makedirs(LOG_DIR, exist_ok=True)

# Hedef sınıf eşleştirmesi: modelin kendi .names'indeki İngilizce isim -> Türkçe etiket
TARGET_CLASSES = {
    "car": "Araba",
    "cars": "Araba",
    "motorcycle": "Motor",
...
def get_class_name_from_model(model, cls_id):
    """
    Modelin kendi isim sözlüğünden (model.names) yola çıkarak Türkçe etiket üretir.
    """
    raw_name = model.names.get(int(cls_id), None)
    if raw_name is None:
        # Eğer model.names'de yoksa yedek sabit eşleştirme (senin modelin için)
        yedek = {0: "Araba", 1: "Motor", 2: "Otobus", 3: "Kamyonet"}
        return yedek.get(int(cls_id), f"ID:{cls_id}")
    # Modelin ismini küçük harfe çevir
    name_lower = str(raw_name).lower().strip()
    # Hedef listesinde var mı kontrol et
    for eng, tr in TARGET_CLASSES.items():
        if name_lower == eng:
            return tr
    # Hedef sınıf değilse orijinal ismi döndür (ör. 'person', 'traffic light')
    return str(raw_name)
...
class LKWrapper:
    ...
    def update(self, frame):
        # 1. Modelin ham tespitlerini al (maske dedektör içinde uygulanır)
        ...
        # 4. Sonuçları derle (her track için en yakın tespiti kullanarak sınıf/conf ekle)
        tracked_objects = {}
        for tid, box in self.boxes.items():
    ...
    with open(log_path, 'w', encoding='utf-8') as log_file:
        ...
        while frame_count < max_frames:
            ...
            for track_id, box_data in tracked.items():
                ...
                # Sınıf ID'sinden Türkçe isim üret
...
def plot_results(results):
    ...
    plt.title('Benzersiz Araç Sayısı (LK)')
    for bar in bars:
        h = bar.get_height()
    ...
    plt.title('Sınıf Dağılımı (Doğru Etiketleme)')
    plt.legend()

    plt.tight_layout()
    plt.savefig('benchmark_lk_dogru.png', dpi=150)
    plt.close()
    print("\n✅ Grafik 'benchmark_lk_dogru.png' olarak kaydedildi.")
...
def main():
    ...
    for name, det in models.items():
        print(f"\n--- {name} test ediliyor (LK, doğru etiket eşleştirmesi) ---")
